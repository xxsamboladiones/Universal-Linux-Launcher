...
export interface ProductStatus { autostart: boolean; appimage: boolean; executable: string }
export interface UpdateStatus { configured: boolean; currentVersion: string; availableVersion: string | null; canInstall: boolean }
...
export const backend = {
  ...
  queueStoreOperation: (provider: StoreId, itemId: string, action: "install" | "update" | "verify") =>
  ...
  rollbackDependency: (id: string) => invoke<void>("rollback_dependency", { id }),
  compatibility: () => invoke<CompatibilityOverview>("get_compatibility_overview"),
