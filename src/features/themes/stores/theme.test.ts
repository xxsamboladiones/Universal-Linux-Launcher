...
const mocks = vi.hoisted(() => ({ listThemes: vi.fn(), getActiveTheme: vi.fn(), setActiveTheme: vi.fn(), importTheme: vi.fn(), removeTheme: vi.fn(), exportTheme: vi.fn() }));
...
describe("theme store", () => {
  beforeEach(() => { vi.clearAllMocks(); vi.stubGlobal("document", { documentElement: { style: { setProperty: vi.fn() }, dataset: {} } }); useThemeStore.setState({ themes: [], active: null, status: "idle", error: null, loaded: false }); });
