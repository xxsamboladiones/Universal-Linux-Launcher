...
export function SettingsPage({
  ...
  const [section, setSection] = useState<
    "library" | "general" | "themes" | "compatibility"
  >("library");
  const [compatibility, setCompatibility] =
    useState<CompatibilityOverview | null>(null);
  const [productStatus, setProductStatus] = useState<ProductStatus | null>(
    null,
  );
  ...
  useEffect(() => {
    if (section === "compatibility")
      void backend.compatibility().then(setCompatibility);
  }, [section]);
    ...
    <div className="settings-page">
        ...
        <span>
          {section === "library"
            ...
            : section === "general"
              ...
              : section === "themes"
                ? "Personalize a aparência do Orbit."
                : "Runtimes e integração do sistema."}
        </span>
      </div>
      <div className="settings-layout">
        <nav className="settings-nav">
          ...
          <button
            className={section === "themes" ? "active" : ""}
          ...
          <button
            className={section === "compatibility" ? "active" : ""}
            onClick={() => setSection("compatibility")}
          >
            <Gauge size={17} />
            Compatibilidade
          </button>
        ...
        {section === "library" ? (
          ...
          <section className="settings-panel general-settings">
            ...
            <label>
              <div>
                <strong>Iniciar com o sistema</strong>
                <small>Abre o Orbit oculto na bandeja do KDE.</small>
              </div>
              <input
                type="checkbox"
                checked={productStatus?.autostart ?? false}
                onChange={(event) =>
                  void backend
                    ...
                    .then(() =>
                      setProductStatus((status) =>
                        status
                          ? { ...status, autostart: event.target.checked }
                          : status,
                      ),
                    )
                }
            ...
            <div className="backup-actions">
              <div>
                <strong>Backup e importação</strong>
                <small>
                  Snapshot consistente da biblioteca e configurações.
                </small>
              </div>
              <button
                onClick={() =>
                  void (async () => {
                    const path = await save({
                      title: "Exportar backup do Orbit",
                      defaultPath: "orbit-backup.orbitbackup",
                      filters: [
                        { name: "Backup Orbit", extensions: ["orbitbackup"] },
                      ],
                    });
                    if (path) {
                      await backend.exportBackup(path);
                      setProductMessage("Backup exportado com sucesso.");
                    }
                  })()
                }
              >
                ...
                onClick={() =>
                  void (async () => {
                    const path = await open({
                      title: "Importar backup do Orbit",
                      multiple: false,
                      directory: false,
                      filters: [
                        { name: "Backup Orbit", extensions: ["orbitbackup"] },
                    ...
                    if (
                      path &&
                      window.confirm(
                        "A biblioteca atual será substituída. Continuar?",
                      )
                    ) {
                      await backend.importBackup(path);
                      setProductMessage("Backup importado. Reinicie o Orbit.");
                    }
                  })()
                }
              >
            ...
            <div className="backup-actions">
              <div>
                <strong>Atualizações assinadas</strong>
                <small>
                  {updateStatus?.availableVersion
                    ? `Versão ${updateStatus.availableVersion} disponível`
                    : updateStatus?.configured
                      ? `Orbit ${updateStatus.currentVersion} está atualizado`
                      : "Canal não configurado neste build"}
                </small>
              </div>
              <button
                onClick={() =>
                  void backend
                    .checkUpdates()
                    .then(setUpdateStatus)
                    .catch((error) => setProductMessage(String(error)))
                }
              >
                Verificar
              </button>
              {updateStatus?.availableVersion && updateStatus.canInstall && (
                <button
                  onClick={() =>
                    void backend
                      .installUpdate()
                      .then(() =>
                        setProductMessage(
                          "Atualização instalada. Reinicie o Orbit.",
                        ),
                      )
                  }
                >
          ...
          <section className="settings-panel compatibility-panel">
            <div className="panel-heading">
              <div>
                <h2>CachyOS e Wayland</h2>
                <p>Componentes detectados no host e runtimes disponíveis.</p>
              </div>
            </div>
            {!compatibility ? (
              ...
              <>
                <div className="compat-status">
                  {(
                    [
                      ...
                      ["DXVK", compatibility.dxvk],
                      ["VKD3D", compatibility.vkd3d],
                      ["Wayland", compatibility.wayland],
                    ] as [string, boolean][]
                  ).map(([name, ok]) => (
                    <span className={ok ? "available" : "missing"} key={name}>
                      {ok ? "✓" : "—"} {name}
                    </span>
                  ))}
                </div>
                <div className="system-summary">
                  <strong>
                    {compatibility.desktop || "Desktop não identificado"}
                  </strong>
                  <small>
                    Sessão {compatibility.sessionType || "desconhecida"} ·
                    Terminal {compatibility.terminal ?? "não detectado"}
                  </small>
                </div>
                <h3>Runtimes</h3>
                <div className="runtime-list">
                  {compatibility.runtimes.length ? (
                      ...
                      <article key={runtime.id}>
                        <div>
                          ...
                          <small>
                            {runtime.family} ·{" "}
                            {runtime.managed
                              ? "gerenciado pelo Orbit"
                              : "instalação externa"}
                          </small>
                        </div>
                        <button
                          onClick={() => void backend.openPath(runtime.path)}
                        >
                          Abrir pasta
                        </button>
                      </article>
                    ...
                    <p>
                      Nenhum Wine/Proton detectado nos diretórios suportados.
                    </p>
                  )}
                </div>
                <button
                  className="icon"
                  onClick={() =>
                    void backend.openPath(compatibility.prefixRoot)
                  }
                >
                  <Library size={16} />
                  Abrir prefixos gerenciados
                </button>
              </>
            )}
          </section>
